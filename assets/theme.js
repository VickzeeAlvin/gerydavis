...
(function(){
  ...
  theme.VariantAvailability = (function() {
    ...
    $('[name="size"] .variant__button-label.disabled').each(function(){
      $(this).parents('.variant-input').find('.unavail_option').addClass('disabled')
    });
      ...
      enableVariantOption: function(group, obj) {
        ...
        if (this.type === 'dropdown') {
          ...
          label.classList.remove(classes.disabled);
           ...
           if(buttonGroup.querySelector('.unavail_option') != null){
           	buttonGroup.querySelector('.unavail_option').classList.remove(classes.disabled);
           }
  
          // Variant sold out - cross out option (remains selectable)
          if (obj.soldOut) {
            if(buttonGroup.querySelector('.unavail_option') != null){
            	buttonGroup.querySelector('.unavail_option').classList.add(classes.disabled);
            }
  ...
  theme.MobileNav = (function() {
    ...
    MobileNav.prototype = Object.assign({}, MobileNav.prototype, {
      init: function() {
        ...
        document.getElementById("head_sub_menu_backbtn").addEventListener('click', this.mainback.bind(this));
      ...
      close: function(evt, noAnimate) {
        ...
        $('#head_sub_menu_backbtn').attr('data-parent', '')
        $('.slide-nav__wrapper').attr('data-level', 1)
        $('body').removeClass('sub_menu_open')
        this.config.menuLevel = 1;
        this.wrapper.removeAttribute('style');
        this.nav.querySelectorAll(selectors.childList).forEach(list => {
          list.classList.remove(classes.isActive);
          list.classList.remove('iam_active')
        });
      ...
      toggleSubNav: function(evt) {
        ...
        this.goToSubnav(btn.dataset.target);
        $('#head_sub_menu_backbtn').attr('data-parent', btn.dataset.target)

      },


      mainback: function(evt){
        var btn = evt.currentTarget;
        this.goToMainSubnav(btn);
      },

      goToMainSubnav: function(target){
        var level = target.dataset.level ;
        var parent = target.dataset.parent
        var targetMenu = document.querySelector('.slide-nav__dropdown' + '[data-parent="'+ parent +'"]');

        targetMenu.classList.remove(classes.isActive);
        targetMenu.classList.remove('iam_active')
        this.config.menuLevel = (+targetMenu.dataset.level - 1);
        var pt = $('ul.slide-nav__dropdown.iam_active.is-active').last().attr('data-parent') ;
        if(pt)
        {
          $('#head_sub_menu_backbtn').attr('data-parent', pt)
          parent = target.dataset.parent
          targetMenu = document.querySelector('.slide-nav__dropdown' + '[data-parent="'+ parent +'"]');
          this.setWrapperHeight(targetMenu.offsetHeight);
        }
        else{
          $('#head_sub_menu_backbtn').attr('data-parent', '')
      ...
      goToSubnav: function(target) {
        ...
        if (targetMenu) {
          targetMenu.classList.add('iam_active')
          this.config.menuLevel = targetMenu.dataset.level;
...
//         console.log(2 ,'el', el ,'parent', parent,'newUrl', newUrl )
...
//   })();
      ...
      updateCartButton: function(evt) {
        ...
        if (variant) {
          if (variant.available) {
            ...
            $('.bold_clone').removeAttr('disabled');
            $('.bold_clone span').text('Add to cart');
          } else {
            // Sold out, disable the submit button and change text
            cartBtn.classList.add(classes.disabled);
            cartBtn.disabled = true;
            cartBtnText.textContent = theme.strings.soldOut;
            $('.bold_clone').attr('disabled','disabled');
            $('.bold_clone span').text('Sold Out');
          }
        } else {
  ...
  DOMready(function(){
    ...
    theme.sections.register('collection-template', theme.Collection);
        theme.sections.register('collection-template', theme.CollectionSidebar);
